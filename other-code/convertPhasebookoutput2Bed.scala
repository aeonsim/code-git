...
val input = new BufferedReader(new FileReader("50k.plus.HDsub.Seq.vcf"))
val phased = new BufferedReader(new FileReader("phases"))
...
var names = new Array[Int](cline.size - 9)

//Store Names
for (cAn <- 9 to (cline.size - 1)){
//for (cAn <- 9 to (cline.size)){
names(cAn - 9) = cline(cAn).toInt
}
