		...
		while (reps < replications) {
			...
			if (mosaic){
				...
				if (reps < 4){
					...
					if (reps == (replications - 1) && indv2 < (male2female * popsize)) cells = cells.map(e => mutGen(spermRate.sample) ::: e) ::: cells.map(e => mutGen(spermRate.sample) ::: e) else cells = cells.map(e => mutGen(baseRate.sample) ::: e) ::: cells.map(e => mutGen(baseRate.sample) ::: e)
				}
				//reps += 1
			} else {
	...
	if (curGen == 0){
		...
		while(indv < popsize){
			...
			while (mutnum < nextGen(indv).size){
				if (muts.contains(nextGen(indv)(mutnum)._1)){
						muts(nextGen(indv)(mutnum)._1) += nextGen(indv)(mutnum)._2
					} else {
						muts +=  nextGen(indv)(mutnum)._1 -> nextGen(indv)(mutnum)._2
					}
				mutnum += 1
		...
		for (m <- muts){
			...
			if (m._2 == (2*popsize)) {
				...
				if (m._1 < 0) mosMutFix = mutAge(m._1) :: mosMutFix else nmMutFix = mutAge(m._1) :: nmMutFix
				println(s"${curGen}\t${if( m._1 < 0 )"m" else "nm"}\t${mutAge(m._1)}\t${if(m._1 < 0 ) mosMutFix.size else nmMutFix.size}\t${if(m._1 < 0 ) mosMutFix.sum/mosMutFix.size.toDouble else nmMutFix.sum/nmMutFix.size.toDouble}\t${if(m._1 < 0 ) mosMutFix.sorted.apply(mosMutFix.size/2) else nmMutFix.sorted.apply(nmMutFix.size/2)}")
...
System.err.println(mosMutFix.sum/mosMutFix.size.toDouble + "\t" + nmMutFix.sum/nmMutFix.size.toDouble)
