...
object targetMobileElementsBreaks{
...
//		val inputBreak = htsjdk.samtools.SamReaderFactory.make.validationStringency(ValidationStringency.SILENT).open(new File(args(1)))
	...
	def bootStrap(reads: Int, evid: Int, geno: Int, avg: Int): Double = {
		...
		for (win <- targetWindows.reverse){
			//print(" " + win._1)		
			//println(s"${win._1} ${win._2} ${win._3} ${input}")
			...
			def getLINEMP(reads: List[String]): List[Int] = {
				if (reads.size >= 1){
					...
					val out = new BufferedWriter(new FileWriter(new File(s"/tmp/${fID}_tmp.fa")))
					reads.foreach(s => out.write(s">R${scala.util.Random.nextInt(100000)}\n${s}\n"))
					//out.write(s">read\n${read}")
					out.close
					val result = (s"blat L1_BT_seq.fasta /tmp/${fID}_tmp.fa stdout" !!).split("\n")
					if (result.size < 6) {
						List(-1)
					} else {
					...
					if (tmp.getProperPairFlag == false && tmp.getMappingQuality >= 20 && tmp.getMappingQuality <= 60){
						...
						if (curMateLTRcheck._1 == true){
							if (windowBoo == false){
								...
								//|| tmp.getSupplementaryAlignmentFlag == false){ Not supllemantary alignment
...
//			breaksWindow.close
			...
			if (windowBoo){
				...
				findOthers(HashMap(win._2 + 490 -> 100,win._3 + 490 -> 100))
				elLength = getLINEMP(mateReads)
				val elementIs = if (elLength.filter(s => s >= 0).size > 0) s"${elLength.filter(s => s >= 0).sorted.head}-${elLength.filter(s => s >= 0).sorted.last}" else "NA"
				...
				//if (fwdP >= 1 && revP >= 1 && (fwdP + revP) >= 3 && splitDif <= 20) {
					//if (fwdP >= 1 && revP >= 1 && (fwdP + revP) >= 3 ) {
						...
						outEvent.write(s"\t${readDepth}:${hetBridge}\t${windowDepth}\t${allIPP - (fwdP+revP)}\t${(fwdP+revP)/(allIPP.toFloat - (fwdP+revP).toFloat + 0.000001)}\t${delDup(c5pSplit,c3pSplit)}\t${best5p}:${best3p}\t${elementIs}\n")
