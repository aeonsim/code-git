...
object findMobileElements{
	...
	def main (args: Array[String]){
		...
		val gap_in = new BufferedReader(new FileReader(new File("/home/aeonsim/refs/gaps.bed")))
		...
		while (gap_in.ready){
			...
			if (prevChrom == tmp(0)){
				...
				gapsChrom += prevChrom -> gapList.reverse.toArray
				gapList = (tmp(1).toInt -1000,tmp(2).toInt +1000) :: Nil
				prevChrom = tmp(0)
			}
		}//end while repeats
...
/* Is Gap? */
		...
		for (chrs <- Chroms){
			...
			while (alignments.hasNext){
				...
				if (candidateWindowEnd != 0 && tmp.getAlignmentStart >= candidateWindowEnd && ((if (best5p == -1 ) true else (isGAP(best5p,gapsChrom(tmp.getReferenceName))._1 == false)) && (if (best3p == -1 ) true else ( isGAP(best3p,gapsChrom(tmp.getReferenceName))._1 == false)))) {
					//println(" no gap: " + isGAP(best5p,gapsChrom(chrs._1)) + " " + isGAP(best3p,gapsChrom(chrs._1)))
					//println( s"${chrs._1}:${candidateWindowStart}-${candidateWindowEnd} " + elLength.filter(s => s > 1).sum + " " + elLength.filter(s => s > 1).size.toDouble + " " + elLength.filter(s => s > 1).sum/elLength.filter(s => s > 1).size.toDouble)
					...
					if (tmp.getProperPairFlag == false && tmp.getMappingQuality >= 20 && tmp.getMappingQuality <= 60 ){
						...
						if (curMainLTRcheck._1 == false && curMateLTRcheck._1 == true){
							...
							if (windowBoo == false){
								...
								if(isGAP(tmp.getAlignmentStart,gapsChrom(tmp.getReferenceName))._1 == true) gapBo = true
								if (gapBo == false) {
									var cMate = tmp 
										try {
									...
									if (tmp.getStringAttribute("SA") == null ) { 
										if (tmp.getReadNegativeStrandFlag == false){ // Not sup and on +ve strand
											...
											if (useMate) mateReads = cMate.getReadString :: mateReads
											fwdP += 1
										} else {
											if (lastfwd) {
