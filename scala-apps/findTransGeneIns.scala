...
object targetMobileElementsExtract{
	...
	def main (args: Array[String]){
		if (args.contains("-h") || args.contains("--help") || args.size < 3){
			println("JAVA_OPTS=-Xmx4g scala -cp htsjdk-1.139.jar:. findMobileElements <SAMPLEBAMs List> <targets file, target Contig name> <Chrs File chr\tstart\tend> ")
			System.exit(0)
		}
		...
		while (inputTargets.ready){
			...
			val chrom = tmp(0).trim
			val start = tmp(1).trim.toInt
			val end = tmp(2).trim.toInt
