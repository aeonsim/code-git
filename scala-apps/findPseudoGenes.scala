...
object findMobileElements{
	...
	def main (args: Array[String]){
		...
		var prevChrom = ""
/* ENSEMBL GENE Build version:
		while (repeats.ready){
			val tmp = repeats.readLine.split("\t")
			if (tmp(0).apply(0) != '#'){
				if (tmp(2) == "exon"){
					val meta = tmp(8).split(";").flatMap(_.split(" "))
					val gID = if (meta.contains("gene_name")) meta(meta.indexOf("gene_name") + 1) else if (meta.contains("gene_id")) meta(meta.indexOf("gene_id") + 1) else "unknown"
					if (prevChrom == ("chr" + tmp(0))){
		...
		while (repeats.ready){
			...
			if (tmp(0).apply(0) != '#'){
				//if (tmp(2) == "exon"){
					val name = tmp(12)
					val gID = tmp(1)
					val exonStarts = tmp(9).split(",")
					val exonEnds = tmp(10).split(",")
					if (prevChrom == (tmp(2))){
						for (i <- 0 to (exonStarts.size - 1 )) repList = (exonStarts(i).toInt,exonEnds(i).toInt,s"${name}_${gID}",s"${name}_${gID}",s"${name}_${gID}",tmp(3)) :: repList
					} else {
						repeatsChrom += prevChrom -> repList.reverse.toArray
						repList = Nil
						for (i <- 0 to (exonStarts.size - 1 )) repList = (exonStarts(i).toInt,exonEnds(i).toInt,s"${name}_${gID}",s"${name}_${gID}",s"${name}_${gID}",tmp(3)) :: repList
		...
		while (repeats.ready){
			...
			if (tmp(0).apply(0) != '#'){
				//if (tmp(2) == "exon"){
					val name = tmp(12)
					val gID = tmp(1)
					val exonStarts = tmp(4).toInt
					val exonEnds = tmp(5).toInt
					if (prevChrom == (tmp(2))){
						repList = (exonStarts,exonEnds,s"${name}_${gID}",s"${name}_${gID}",s"${name}_${gID}",tmp(3)) :: repList
					} else {
						repeatsChrom += prevChrom -> repList.reverse.toArray
						repList = Nil
						repList = (exonStarts,exonEnds,s"${name}_${gID}",s"${name}_${gID}",s"${name}_${gID}",tmp(3)) :: repList
...
/* Is Gap? */
		...
		for (chrs <- Chroms){
			...
			def splitMate(cigarString: String): Boolean = {
				val splits = List("H","S")
				splits.map(a => cigarString.contains(a)).contains(true)
			}
			...
			while (alignments.hasNext){
				...
				//if(! isGAP(tmp.getAlignmentStart,gapsChrom(tmp.getReferenceName))._1){
				...
				if (candidateWindowEnd != 0 && tmp.getAlignmentStart >= candidateWindowEnd && ((
					...
					println( s"${chrs._1}:${candidateWindowStart}-${candidateWindowEnd} ${numReads} ${fwdP} ${revP} ${Pipp} ${fwdS} ${revS} ${mateSR} ${typeEvent.size} avgDif: ${avgSPDif}")
				...
				if (Pipp <= 0.05 ){ //&& bootStrap(numReads,fwdP + revP, expReads, expIPP) <= 0.005){
					...
					println(s"${Pipp <= 0.05} ${(fwdS + revS) > 0} ${mateSR > 0} ${typeEvent.size == 1}")

					if (Pipp <= 0.05 && fwdP > 0 && revP > 0 && (fwdS + revS) > 0 && mateSR >= 0 && typeEvent.size == 1 && avgSPDif < 50) {
					//if (bootStrap(numReads,fwdP + revP + fwdS + revS, expReads, expIPP + expSR) <= 0.05 && (fwdS + revS) > 0 && mateSR >= 4 && typeEvent.size == 1) {
						...
						outEvent.write(s"\t${readDepth}:${hetBridge}\t${windowDepth}\t${allIPP - (fwdP+revP)}\t${(fwdP+revP)/(allIPP.toFloat - (fwdP+revP).toFloat + 0.000001)}\t${delDup(c5pSplit,c3pSplit)}\t${best5p}:${best3p}\t${polyA}\t${elementIs}\t${mateSR}\n")
