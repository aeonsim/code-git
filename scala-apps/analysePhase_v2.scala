...
/* Report identified Trios & there Pedigree Structure*/
	...
	print(s"CHROM\tPOS\t${curTrio}\t${trios(curTrio)._2(0)}\t${trios(curTrio)._2(1)}")
	for (child <- trios("NL288458773")._3) print(s"\t${child}")
	print("\n")
	while (in_vcf.ready){
	val cline = in_vcf.readLine.split("\t")
	val fam = trios(curTrio)
	var gender = if (pedFile(curTrio).apply(4) == 1) "m" else "f"
	val parGeno = cline(vcfanimals(curTrio)).split(":")(0).split("\\|")
	if (parGeno.contains("0") && parGeno.contains("1")) {
	print(s"${cline(0)}\t${cline(1)}\t${cline(vcfanimals(curTrio)).split(':')(0)}\t${cline(vcfanimals(fam._2(0))).split(':')(0)}\t${cline(vcfanimals(fam._2(1))).split(':')(0)}")
	for (child <- fam._3){
		val pgeno = cline(vcfanimals(child)).split(":")(0).split("\\|")
	
		//if (gender == "m") print("\t" + cline(vcfanimals(child)).split(":")(0) + " " + parGeno.indexOf(pgeno(0))) else print( "\t" + cline(vcfanimals(child)).split(":")(0) + " " + parGeno.indexOf(pgeno(1)))
		if (gender == "m")  {
			if (parGeno.indexOf(pgeno(0)) == 0) print("\tF") else print("\tM")
			} else {
				if (parGeno.indexOf(pgeno(1)) == 0) print("\tF") else print("\tM")
				}
	}
	print("\n")
	}
